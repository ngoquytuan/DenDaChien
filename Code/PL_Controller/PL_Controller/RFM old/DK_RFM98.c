...
extern unsigned char Tx_request;
/************************Description************************
                      ________________
                     /                \
                    /      HOPERF      \
...
{ 
  ...
  {
// 	  writeStringUART("gb_WaitStableFlag\r\n");
    if(gb_SysTimer_1S!=0)
    {
...
// 	toStr(str_buffer, gb_RxData[i]);writeStringUART(" ");writeMStringUART(str_buffer);
//	return 1;
...
// 	  toStr(str_buffer, temp);writeStringUART(" ");writeMStringUART(str_buffer);
      if(temp==21)
      {
        gb_WaitStableFlag=1; 
      }
...
void RFM98_TestConfig(void)
{
  ...
  PORTB&=~(1<<2);                                       //Ver1.1 (Change for Reset)
  for(i=200;i!=0;i--)                                      //Delay
    asm("NOP"); 
  PORTB|=(1<<2); 
  for(i=200;i!=0;i--)                                      //Delay
