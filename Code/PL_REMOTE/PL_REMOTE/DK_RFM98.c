...
// 	gw_RF_SentInterval=C_RF_SentInterval;                    //RF parameter Send interval time(ms)
	...
	else if(RF_ModemBuf[gb_Modem_Addr]==C_SysMode_LoRa)
	{
		...
		gb_BandBuf_Addr=0;//Bandwidth = 7.8kkHz
	}
...
**Output:   None
**********************************************************/
void RFM98_EntryLoRa(void)									//Lora mode=0x80, Low Frequency mode=0x08
{
...
**Output:   None
**********************************************************/
void RFM98_LoRaClearIrq(void)								//Clear all flag
{
...
void RFM98_LoRaEntryRx(void)
{
  ...
  SPIWrite(LR_RegPayloadLength+ 21);                       //RegPayloadLength  21byte(this register must define when the data long of one byte in SF is 6)
...
void RFM98_Config(u8 mode)
{
  ...
  HOLD_ON_RESET;                                       //Ver1.1 (Change for Reset)
