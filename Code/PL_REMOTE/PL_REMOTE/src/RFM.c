...
{
  SPI(LR_RegOpMode+0x01+0x08);                              //Standby
}
...
{
  SPI(LR_RegOpMode+0x00+0x08);                              //Sleep
}
...
void RFM98_LoRaEntryRx(void)
{
  ...
  SPI(0x4D00+0x84);                                   //Normal and Rx
  SPI(LR_RegHopPeriod+0xFF);                          //RegHopPeriod NO FHSS
  SPI(REG_LR_DIOMAPPING1+0x01);                       //DIO0=00, DIO1=00, DIO2=00, DIO3=01
      
  SPI(LR_RegIrqFlagsMask+0x3F);                       //Open RxDone interrupt & Timeout
  RFM98_LoRaClearIrq();   
  
  SPI(LR_RegPayloadLength+ 21);                       //RegPayloadLength  21byte(this register must difine when the data long of one byte in SF is 6)
    
  addr = SPI((u8)(LR_RegFifoRxBaseAddr>>8));           //Read RxBaseAddr
  SPI(LR_RegFifoAddrPtr+addr);                        //RxBaseAddr -> FiFoAddrPtr¡¡ 
  SPI(LR_RegOpMode+0x05+0x08);                        //Continuous Rx Mode
...
{ 
  if(gb_WaitStableFlag==0x0f) 
  {
    if(gb_SysTimer_1S!=0)
    {
      if((SPI((u8)(LR_RegModemStat>>8))&0x04)==0x04)   //Rx-on going RegModemStat
      {
        gb_WaitStableFlag=1;        
      }
    }
...
{
  u16 temp=10;
  temp=SPI((u8)(LR_RegRssiValue>>8));                  //Read RegRssiValue£¬Rssi value
...
u8 RFM98_LoRaRxPacket(void)
{
  ...
  if(gb_StatusRx==1)
  {
    ...
    addr = SPI((u8)(LR_RegFifoRxCurrentaddr>>8));      //last packet addr
    SPI(LR_RegFifoAddrPtr+addr);                      //RxBaseAddr -> FiFoAddrPtr    
    if(RFM98SpreadFactorTbl[gb_RateBuf_Addr]==6)           //When SpreadFactor is six£¬will used Implicit Header mode(Excluding internal packet length)
      ...
      packet_size = SPI((u8)(LR_RegRxNbBytes>>8));     //Number for received bytes    
...
void RFM98_LoRaEntryTx(void)
{
  ...
  SPI(0x4D00+0x87);                                   //Tx for 20dBm
  SPI(LR_RegHopPeriod);                               //RegHopPeriod NO FHSS
  SPI(REG_LR_DIOMAPPING1+0x41);                       //DIO0=01, DIO1=00, DIO2=00, DIO3=01
  
  RFM98_LoRaClearIrq();
  SPI(LR_RegIrqFlagsMask+0xF7);                       //Open TxDone interrupt
  SPI(LR_RegPayloadLength+ 21);                       //RegPayloadLength  21byte
  
  addr = SPI((u8)(LR_RegFifoTxBaseAddr>>8));           //RegFiFoTxBaseAddr
  SPI(LR_RegFifoAddrPtr+addr);                        //RegFifoAddrPtr
...
void RFM98_LoRaTxWaitStable(void)
{
  ...
  if(gb_WaitStableFlag==0x0f) 
  {
    if(gb_SysTimer_1S!=0)
    {
      temp=SPI((u8)(LR_RegPayloadLength>>8));
      if(temp==21)
      {
        gb_WaitStableFlag=1; 
      }
...
{ 
  ...
  if(gb_WaitStableFlag==1)
  {
    ...
    if(gb_StatusTx==0)                                     //First entry Tx mode
    {
      ...
      SPI(LR_RegOpMode+0x03+0x08);                    //Tx Mode           
      TxFlag=1;
      gb_WaitStableFlag=3;
    }
  }
...
//   Input_RFData();
  ...
  RFM98_Config(1);  
  for(i=0;i<2;i++)
    SPI(RFM98FSKRateTbl[gb_RateBuf_Addr][i]);         //setting rf rate parameter  
...
//   RFM98_PORchk_In;  
  ...
  if(!mode)
  {
    ...
    {
      SPI(RFM98FreqTbl[gb_FreqBuf_Addr][i]);  
    }

    //setting base parameter 
    SPI(RFM98PowerTbl[gb_PowerBuf_Addr]);             //Setting output power parameter  
    
    SPI(LR_RegOcp+0x0B);                              //RegOcp,Close Ocp
    SPI(LR_RegLna+0x23);                              //RegLNA,High & LNA Enable
    
    if(RFM98SpreadFactorTbl[gb_RateBuf_Addr]==6)           //SFactor=6
    {
      u8 tmp;
      SPI(LR_RegModemConfig1+(RFM98LoRaBwTbl[gb_BandBuf_Addr]<<4)+(CR<<1)+0x01);//Implicit Enable CRC Enable(0x02) & Error Coding rate 4/5(0x01), 4/6(0x02), 4/7(0x03), 4/8(0x04)
      SPI(LR_RegModemConfig2+(RFM98SpreadFactorTbl[gb_RateBuf_Addr]<<4)+(CRC<<2)+0x03);
    ...
    {
      SPI(LR_RegModemConfig1+(RFM98LoRaBwTbl[gb_BandBuf_Addr]<<4)+(CR<<1)+0x00);//Explicit Enable CRC Enable(0x02) & Error Coding rate 4/5(0x01), 4/6(0x02), 4/7(0x03), 4/8(0x04)
      SPI(LR_RegModemConfig2+(RFM98SpreadFactorTbl[gb_RateBuf_Addr]<<4)+(CRC<<2)+0x03);  //SFactor &  LNA gain set by the internal AGC loop 
    }
    SPI(LR_RegSymbTimeoutLsb+0xFF);                   //RegSymbTimeoutLsb Timeout = 0x3FF(Max) 
    
    SPI(LR_RegPreambleMsb + 0);                       //RegPreambleMsb 
    SPI(LR_RegPreambleLsb + 12);                      //RegPreambleLsb 8+4=12byte Preamble
    
    SPI(REG_LR_DIOMAPPING2+0x01);                     //RegDioMapping2 DIO5=00, DIO4=01
